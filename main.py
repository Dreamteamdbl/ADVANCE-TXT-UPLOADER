...
OWNER_ID = 6133985472 # Replace with the actual owner's user ID

# List of sudo users (initially empty or pre-populated)
SUDO_USERS = [6133985472,1265309848,530148656]

AUTH_CHANNEL = -1002565842447
...
async def upload(bot: Client, m: Message):
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            if "visionias" in url:
            ...
            elif "tencdn.classplusapp" in url or "media-cdn-a.classplusapp.com" in url or "media-cdn-alisg.classplusapp.com" in url or "videos.classplusapp" in url or "media-cdn.classplusapp" in url:
             ...
             url = response.json()['url']
                ...
                url = url.replace("https://appx-transcoded-videos-mcdn.akamai.net.in/videos/bboosteracademy-data/", "")
                name1 = links[i][0].replace("\t", "").replace(":", "").replace("/", "").replace("+", "").replace("#", "").replace("|", "").replace("@", "@").replace("*", "").replace(".", "").replace("https", "").replace("http", "").strip()
                name = f'{str(count).zfill(3)}) {name1[:60]}'
                cmd = f'yt-dlp -o "{name}.mp4" "{url}"'
